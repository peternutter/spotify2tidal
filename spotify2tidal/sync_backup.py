...
async def export_backup(engine) -> dict:
    ...
    engine._log("progress", "Fetching Spotify library snapshot...")
    spotify_tracks = await engine.spotify_fetcher.get_saved_tracks(
        limit=engine._item_limit
    )
    spotify_albums = await engine.spotify_fetcher.get_saved_albums(
        limit=engine._item_limit
    )
    spotify_artists = await engine.spotify_fetcher.get_followed_artists(
        limit=engine._item_limit
    )
    spotify_podcasts = await engine.spotify_fetcher.get_saved_shows(
        limit=engine._item_limit
    )

    engine.library.tracks = spotify_tracks
    engine.library.albums = spotify_albums
    engine.library.artists = spotify_artists
    engine.library.podcasts = spotify_podcasts
